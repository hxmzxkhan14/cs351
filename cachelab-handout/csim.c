...
// 64 bit variable type that will be used to hold memory address
...
struct Cache build(long long totalSets, int totalLines, int blockSize) {
	...
	// allocat memory for the new cache based off how many total sets it will hace
...
int findLRU(struct cacheSet set, struct Parameters param){
	...
	// and it's number of uses for comparison with the rest of the set, replacing it if another line
...
// checks whether any lines in a given set are empty and returns the number of empty lines if one exists
// if there are none then return 0 because then the set is full
int anyEmptyLines(struct cacheSet set, struct Parameters param) {
	...
	for (int i = 0; i < totalLines; i++) {
		line = set.lines[i];
		// if a line is not valid, return it's line number
		if(line.valid == 0){
			return i;
		}
	}
...
struct Parameters Simulation(struct Cache aCache, struct Parameters param, memAddress address){
	// calculate the size of the tag by sbtracting the
	...
	for (int i = 0; i < param.E; i++){
		...
		// there was a hit, increase the hit and uses counter, also change the flag
...
// if there was a miss and the set is not full then place the miss in the nearest empty line
...
/* otherwise we have to evict */
	...
	// need to evict the LRU to make space for the miss
